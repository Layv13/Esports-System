package esports.view.admin;

import esports.model.Session;
import esports.view.*;
import esports.view.admin.ChangePasswordPanel;
import javax.swing.*;
import java.awt.*;

public class AdminDashboard extends JPanel {

    private static final String CARD_OVERVIEW  = "OVERVIEW";
    private static final String CARD_REGISTER  = "REGISTER";
    private static final String CARD_BRACKET   = "BRACKET";
    private static final String CARD_TEAMS     = "TEAMS";
    private static final String CARD_RESULTS   = "RESULTS";
    private static final String CARD_CHANGEPASS = "CHANGEPASS";

    private final MainFrame        mainFrame;
    private final CardLayout       contentLayout = new CardLayout();
    private final JPanel           contentPanel  = new JPanel(contentLayout);

    private AdminOverviewPanel  overviewPanel;
    private RegisterManagerPanel registerPanel;
    private BracketPanel        bracketPanel;
    private TeamsPanel          teamsPanel;
    private MatchesPanel        matchesPanel;
    private JLabel              headerTitle;

    public AdminDashboard(MainFrame mainFrame) {
        this.mainFrame = mainFrame;
        setLayout(new BorderLayout());
        setBackground(UIConstants.BG_DARK);
        buildUI();
    }

    private void buildUI() {
        add(buildHeader(), BorderLayout.NORTH);
        ChangePasswordPanel changePassPanel = new ChangePasswordPanel();
        contentPanel.add(changePassPanel, CARD_CHANGEPASS);

       
        String[][] navItems = {
            {"  Overview",          CARD_OVERVIEW},
            {"  Register Manager",  CARD_REGISTER},
            {"  Bracket",           CARD_BRACKET},
            {"  All Teams",         CARD_TEAMS},
            {" Match Results",     CARD_RESULTS},
            {"  Change Password", CARD_CHANGEPASS},
        };
        SidebarPanel sidebar = new SidebarPanel(
            "ADMIN", key -> navigate(key), navItems,
            () -> { Session.getInstance().logout(); mainFrame.showLogin(); }
        );
        add(sidebar, BorderLayout.WEST);

        // Content area
        contentPanel.setBackground(UIConstants.BG_DARK);
        overviewPanel  = new AdminOverviewPanel();
        registerPanel  = new RegisterManagerPanel(this);
        bracketPanel   = new BracketPanel();
        teamsPanel     = new TeamsPanel();
        matchesPanel   = new MatchesPanel();

        contentPanel.add(overviewPanel,  CARD_OVERVIEW);
        contentPanel.add(registerPanel,  CARD_REGISTER);
        contentPanel.add(bracketPanel,   CARD_BRACKET);
        contentPanel.add(teamsPanel,     CARD_TEAMS);
        contentPanel.add(matchesPanel,   CARD_RESULTS);

        add(contentPanel, BorderLayout.CENTER);
        navigate(CARD_OVERVIEW);
    }

    private JPanel buildHeader() {
        JPanel header = new JPanel(new BorderLayout());
        header.setBackground(UIConstants.BG_HEADER);
        header.setPreferredSize(new Dimension(0, UIConstants.HEADER_H));
        header.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, 0, 1, 0, UIConstants.BORDER),
            BorderFactory.createEmptyBorder(0, 24, 0, 24)
        ));

        headerTitle = new JLabel("Overview");
        headerTitle.setFont(UIConstants.FONT_LARGE);
        headerTitle.setForeground(UIConstants.TEXT_PRIMARY);
        header.add(headerTitle, BorderLayout.WEST);

        String name = Session.getInstance().getCurrentUser().getFullName();
        JLabel user = new JLabel(" " + name + "  [Admin]");
        user.setFont(UIConstants.FONT_BODY);
        user.setForeground(UIConstants.TEXT_SECOND);
        header.add(user, BorderLayout.EAST);
        return header;
    }

    public void navigate(String card) {
        contentLayout.show(contentPanel, card);
        switch (card) {
            case CARD_OVERVIEW: headerTitle.setText("Overview");          overviewPanel.refresh();  break;
            case CARD_REGISTER: headerTitle.setText("Register Manager");  registerPanel.refresh();  break;
            case CARD_BRACKET:  headerTitle.setText("Tournament Bracket");bracketPanel.refresh();   break;
            case CARD_TEAMS:    headerTitle.setText("All Teams");         teamsPanel.refresh();     break;
            case CARD_RESULTS:  headerTitle.setText("Match Results");     matchesPanel.refresh();   break;
            case CARD_CHANGEPASS: 
    headerTitle.setText("Change Password"); 
    break;
        }
    }

    public void refresh() {
        overviewPanel.refresh();
        bracketPanel.refresh();
        teamsPanel.refresh();
        matchesPanel.refresh();
        registerPanel.refresh();
    }
}
